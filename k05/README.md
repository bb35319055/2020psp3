...
l.111~113: visitedを初期化

l.115: スタックを初期化

l.116: 出発点をPushにする

l.119: whileでスタックが空になるまで繰り返す

l.120: xにスタックからポップした値を代入する

l.122~131: visitedのx番目の値が0であるならば1にして、その地点から行けるところをスタックに入れる

l.199: 変数をint型で宣言

l.200~202: visitedを初期化

l.204: キューを初期化

l.205: 出発点をキューに入れる

l.208: whileでキューが空になるまで繰り返す

l.209: xにキューから取り出した値を代入する

l.211~220: visitedのx番目の値が0であるならば1にして、その地点から行けるところをキューに入れる
